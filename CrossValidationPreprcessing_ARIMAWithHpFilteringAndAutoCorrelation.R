...
#outputFile="E:/Research with dr park/IJKAI/TSP-master/TSP-master/outputMean.out"
...
# 
  for(j in 1:length(files.sortednames)){
     print(files.sortednames[j])

    data <- read.table(files.sortednames[j],sep=" ",
    ...
    print(returnIndex)
    print(n)
    data<-ts(dataGrouped[,2],start=c(dataGrouped[1,2]),frequency=length(dataGrouped$V1))
    if(n-returnIndex>14){
  ...
  cat(paste(files.sortednames[j],mean(mae),collapse = " "), file=outputFile, append=TRUE, sep = "\n")
